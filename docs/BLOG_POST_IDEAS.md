...
| SOLID: Liskov Substitution Principle na prática (React) | Background técnico | ✅ Publicado — terceiro post da série (L), continua o slot aberto no post de OCP: três extensões com a mesma assinatura e contratos diferentes, contrato nomeado (`ProfileExtension`) e suíte de testes de contrato. **Próximo da série: Interface Segregation (I).** |
