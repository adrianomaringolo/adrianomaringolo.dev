...
| SOLID: Open/Closed Principle na prática (React) | Background técnico | ✅ Publicado — segundo post da série (O), continua o mesmo `UserProfile`: prop `variant` com cascata de `if`s refatorada para composição. |
| SOLID: Liskov Substitution Principle na prática (React) | Background técnico | ✅ Publicado — terceiro post da série (L), continua o slot aberto no post de OCP: três extensões com a mesma assinatura e contratos diferentes, contrato nomeado (`ProfileExtension`) e suíte de testes de contrato. |
| SOLID: Interface Segregation Principle na prática (React) | Background técnico | ✅ Publicado — quarto post da série (I), continua o `ProfileExtension` do post de LSP: contrato engordando a cada extensão nova (`permissions`, `layout`), suíte de testes de contrato voltando a precisar de exceção por implementação, e a correção via hooks/contexto pulados por quem precisa. **Próximo da série: Dependency Inversion (D).** |
