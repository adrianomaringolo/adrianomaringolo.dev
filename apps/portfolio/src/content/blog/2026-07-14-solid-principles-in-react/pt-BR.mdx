...
tldr:
  - "SOLID nasceu na POO, mas os problemas que resolve, componentes que fazem coisa demais, acoplamento difícil de trocar, também aparecem em bases de código React que envelhecem."
  - "S — Single Responsibility: um componente, um motivo para mudar."
  - "O — Open/Closed: extensível por composição, sem precisar reabrir código estável."
  - "L — Liskov: componentes com a mesma interface de props devem ser intercambiáveis."
  - "I — Interface Segregation: não force um componente a depender de props que ele não usa."
  - "D — Dependency Inversion: dependa de abstrações (hooks, Context), não de implementações concretas."
---
